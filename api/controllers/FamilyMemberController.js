...
var mongoose = require('mongoose'), path = require('path'), fs = require('fs'), Member = mongoose.model('FamilyMember'), Height = mongoose.model('Height'), City = mongoose.model('City'), Citizenship = mongoose.model('Citizenship'), Native = mongoose.model('Native'), Education = mongoose.model('Education'), config = require('../../Config');
exports.createFamilyMember = function (req, res) {
    var familymember = new Member(req.body), GUID = random.randomBytes(16).toString("hex"), extension = req.body.Filename.slice((req.body.Filename.lastIndexOf(".") - 1 >>> 0) + 2);
