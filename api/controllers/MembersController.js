...
var mongoose = require('mongoose'), Member = mongoose.model('Member'), path = require('path'), fs = require('fs'), config = require('../../Config');
exports.createMember = function (req, res) {
    var newmember = new Member(req.body), GUID = Num.randomBytes(16).toString("hex"), extension = req.body.FileName.slice((req.body.FileName.lastIndexOf(".") - 1 >>> 0) + 2);
...
exports.updateMember = function (req, res) {
    var date = new Date, GUID = Num.randomBytes(16).toString("hex"), extension = req.body.FileName.slice((req.body.FileName.lastIndexOf(".") - 1 >>> 0) + 2), flag = false;
    if (req.body.Image.startsWith(config.nodeURL + '/getStaticImage')) {
        flag = true;
    }
