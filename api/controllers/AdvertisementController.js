var mongoose = require('mongoose'), advertisement = mongoose.model('Advertisement'), advertisementType = mongoose.model('AdvertisementType'), advertisementLocation = mongoose.model('AdvertisementLocation'), advertisementPhoto = mongoose.model('AdvertisementPhotos'), path = require('path'), fs = require('fs'), config = require('../../Config'), moment = require('moment');
var number = require("crypto");
...
exports.createAdvertisement = function (req, res) {
    var newAdvertisement = new advertisement(req.body), extension, newPhoto, guid, AdvertisementDir = ('./Photos/Advertisement');
    newAdvertisement.save(function (err, data) {
        ...
        if (req.body.NoOfFiles == 1) {
            ...
            for (var i = 0; i < req.body.NoOfFiles; i++) {
                guid = number.randomBytes(16).toString("hex");
...
exports.updateAdvertisement = function (req, res) {
    var date = new Date, GUID = number.randomBytes(16).toString("hex"), extension, newPhoto, isPhotos = false;
